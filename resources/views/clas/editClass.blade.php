...
<div class="content-area py-1">
	<div class="container-fluid">
		...
		<div class="box box-block bg-white">
			...
			<form method="post" action="{{ route('updateClas') }}" enctype="multipart/form-data">
				@csrf
				<input type="hidden" name="id" value="{{$clases['id']}}">
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<label>نام صنف <span class="text-danger">*</span></label>
							<input type="text" name="name" class="form-control" required value="{{$clases['name']}}">
						...
						<div class="form-group">
							...
							<select name="subject" class="form-control no-padding" required>
								@foreach($subjects as $subject)
									<option value="{{ $subject->id }}" <?php if ($subject->id == $clases['subject_id']): ?>
										<?php echo 'selected' ?>
									<?php endif ?>>{{ $subject->name }}</option>
								@endforeach
							</select>
							<span class="font-90 text-muted"></span>
						</div>
						...
						<div class="form-group">
							...
							<select name="teacher" class="form-control no-padding" required>
								@foreach($teachers as $teacher)
									<option value="{{ $teacher->id }}" <?php if ($teacher->id == $clases['teacher_id']): ?>
										<?php echo 'selected' ?>
									<?php endif ?>>{{ $teacher->full_name }}</option>
								@endforeach
							</select>
							<span class="font-90 text-muted"></span>
						</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="time" name="start_time" class="form-control" value="{{$clases['start_time']}}">
									<span class="font-90 text-muted"></span>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="time" name="end_time" class="form-control" value="{{$clases['end_time']}}">
									<span class="font-90 text-muted"></span>
								</div>
							</div>
						</div>
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<select class="no-padding form-control" name="month">
										@for($i = 1; $i <= 12; $i++)
											<option value="{{$month[$i]}}" <?php if ($month[$i] == $clases['month']): ?>
												<?php echo 'selected' ?>
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<select name="day" class="form-control" class="no-padding">
										@for($i = 1; $i <= 31; $i++) 
											<option value="{{$i}}" <?php if ($i == $clases['day']): ?>
												<?php echo 'selected' ?>
											<?php endif ?>>{{$i}}</option>
										@endfor
									</select>
						...
						<div class="form-group">
							...
							<select name="status" class="form-control no-padding" required>
								@foreach($status as $key => $value)
									<option value="{{$key}}" <?php if ($key == $clases['status']): ?>
										<?php echo 'selected' ?>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<label>فیس</label>
							<input type="number" name="fee" min="0" class="form-control" value="{{$clases['fee']}}">
							<span class="font-90 text-muted"></span>
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group">
							<label>تخفیف</label>
							<input type="number" name="discount" min="0" class="form-control" value="{{$clases['discount']}}">
				...
				<div class="row">
					<div class="col-md-12">
				        ...
				        <textarea class="form-control" name="description" rows="4">{{$clases['description']}}</textarea>
					</div>
				</div>
